...
export class Product extends AggregateRoot<string> {
  private constructor(
    ...
    private salePrice: bigint,
    private purchaseCost: bigint,
  ...
  updatePrice(salePrice: bigint, purchaseCost = this.purchaseCost): void {
    if (salePrice < 0n || purchaseCost < 0n) throw new InvalidValueError('Invalid price');
